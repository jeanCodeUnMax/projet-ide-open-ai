...
test('le bootstrap installe tous les preloads modernes avant main sans bloquer app.whenReady', async () => {
  const source = await readFile(bootstrapUrl, 'utf8')
  assert.match(source, /const\s+SESSION_PRELOADS\s*=\s*Object\.freeze\(\[/)
  assert.match(source, /session\.defaultSession\.getPreloadScripts\(\)/)
  assert.match(source, /for\s*\(const preload of SESSION_PRELOADS\)/)
  assert.match(source, /session\.defaultSession\.registerPreloadScript\(\{/)
  assert.match(source, /type:\s*['"]frame['"]/)
  assert.match(source, /id:\s*preload\.id/)
  assert.match(source, /filePath:\s*preload\.filePath/)
  ...
  assert.match(source, /openfox-workflow-designer-preload\.cjs/)
  assert.match(source, /ide-open-ai-openfox-local-session/)
  assert.match(source, /ide-open-ai-openfox-workflow-designer/)
